import { IQuestion, IQuestionDTO, IResult, ISession, ISessionDTO, QuestionCategory, QuestionDifficulty } from "../../../../shared/src";
...
describe('Testing the questionService', () => {
    ...
    describe('Test getAll', () => {
        it('Should return all questions', async () => {
            mockQuestionRepo.getAll = jest.fn().mockReturnValueOnce(Promise.resolve(questions))
            service = new QuestionService(mockQuestionRepo, mockSessionRepo)
    ...
    describe('Test saveResults', () => {
        it('should create the session', async () => {
            mockQuestionRepo.getByIds = jest.fn().mockReturnValueOnce(Promise.resolve(questions))
            const sessionId = v4()
            const expertQuestions = [...questions.filter((question: IQuestion) => question.difficulty === QuestionDifficulty.EXPERT)].slice(0, 2)
            const sessionDTO: ISessionDTO = {
                sessionId: sessionId,
                level: QuestionDifficulty.DEBUTANT,
                answers: [
                    {
                        questionId: expertQuestions.length ? expertQuestions[0]._id : '',
                        answerGiven: expertQuestions.length ? expertQuestions[0].correctAnswer : 0,
                        correct: true
                    },
                    {
                        questionId: expertQuestions.length ? expertQuestions[1]._id : '',
                        answerGiven: expertQuestions.length ? expertQuestions[1].correctAnswer : 0,
                        correct: true
                    }
                ]
            }
    ...
    describe('Test getResults', () => {
        it('should return a 404 error', async () => {
            mockSessionRepo.get = jest.fn().mockReturnValueOnce(Promise.resolve(undefined))
            expect(service.getResults(v4())).rejects.toThrow(HttpError)
        })
        
        it('should return the results', async () => {
            const expertQuestions: IQuestion[] = [...questions.filter((question: IQuestion) => question.difficulty === QuestionDifficulty.EXPERT && question.category === QuestionCategory.CONJUGAISON)].slice(0, 2)
            const intermediateQuestion = questions.find((question: IQuestion) => question.difficulty === QuestionDifficulty.INTERMEDIAIRE && question.category === QuestionCategory.CONJUGAISON) as unknown as IQuestion
            const beginnerQuestion: IQuestion = questions.find((question: IQuestion) => question.difficulty === QuestionDifficulty.DEBUTANT && question.category === QuestionCategory.CONJUGAISON) as unknown as IQuestion
            const sessionId = v4()
            ...
            mockSessionRepo.get = jest.fn().mockReturnValueOnce(Promise.resolve(session))
            mockSessionRepo.getPctCorrectAnswersByCategoryAndDifficulty = jest.fn().mockReturnValue(100)
