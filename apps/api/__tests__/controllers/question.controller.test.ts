...
import { IQuestion, IQuestionDTO, IResult, QuestionDifficulty, SaveResultsValidationSchema } from '../../../../shared/src';
...
describe('Testing the questionController', () => {
    ...
    describe('getAll', () => {
        it('should return all questions', async () => {
            mockService.getAll = jest.fn().mockReturnValueOnce(Promise.resolve(questions))
            controller = new QuestionController(mockService)
        ...
        it('should return an empty list', async () => {
            mockService.getAll = jest.fn().mockReturnValueOnce(Promise.resolve([]))
            controller = new QuestionController(mockService)
            const questionsList: IQuestion[] = await controller.getAll()
    ...
    describe('saveResults', () => {
        ...
        it('should return the session', async () => {
            ...
            mockService.saveResults = jest.fn().mockReturnValueOnce(Promise.resolve(session))
            controller = new QuestionController(mockService)
            const response = await controller.saveResults(session as SaveResultsValidationSchema)
    ...
    describe('getResults', () => {
        it('should return the results', async () => {
            ...
            mockService.getResults = jest.fn().mockReturnValueOnce(Promise.resolve(results))
            controller = new QuestionController(mockService)
            const response = await controller.getResults(sessionId)
            expect(response).toBeTruthy()
            expect(response.session.sessionId).toBe(sessionId)
            expect(response.globalPctConjugaison).toBe(results.globalPctConjugaison)
        })
    })
})